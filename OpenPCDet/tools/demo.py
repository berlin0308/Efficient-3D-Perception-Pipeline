...
def parse_config():
    ...
    parser.add_argument('--no_viz', action='store_true', help='skip GUI visualization (e.g. for headless); only run inference and print counts')
    parser.add_argument('--save_image', type=str, default=None, help='save screenshot of the scene to this path (e.g. demo_out.png). With --no_viz, only tries to save (may fail without display).')
            ...
            if not args.no_viz or save_path:
                try:
                    V.draw_scenes(
                        ...
                        save_path=save_path, run_interactive=not args.no_viz
                    )
                    if save_path:
